...
| 9 | Replace the starter IVR in `ServiceDesk` with **Virtual Agent V2** and connect handled, escalated, errored, and queue-failure outcomes. | `ServiceDesk` version 5 is published as **Latest**, with zero validation errors. Active `Entry Point-1` routes to `ServiceDesk` **Latest**. A real version 5 call at 20:48 tenant time followed `NewContact → AIAgent → DisconnectContact → ContactEnded`; a same-time AI Agent Studio Voice session ran `lookup_order` successfully, and Analyze showed one execution with zero node errors. The session was matched by time, not by a shared interaction ID. Spoken wording and the separate human-handoff call still need confirmation. |
