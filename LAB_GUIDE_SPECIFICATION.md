...
2. `ServiceDesk` versions 1 and 4 have no active version label. Control Hub's entry-point picker routes by label, so it cannot select either historical version directly. Separately named copies of the supplied version 1 and version 4 exports were imported through **Control Hub → Contact Center → Customer Experience → Flows → Manage Flows → Import**. The original `ServiceDesk` and its **Latest** label were not changed. Version 1's exported bearer values were removed from the local import file; its temporary credential was entered only in the assigned Flow Designer UI. The isolated direct-REST copy passed Validation and has published version 2 on **Latest** and **Test**. The isolated corrected-menu copy passed Validation and has published version 1 on **Latest** and **Test**. [Direct-REST QA version history](docs/assets/lab-guide/live/cp3-v1-qa-published.jpg) and [corrected-menu QA version history](docs/assets/lab-guide/live/cp2-v4-qa-published.jpg) record publication, not caller behavior.
...
On September 25, the entry point was temporarily set to the [isolated version 4 QA flow on Latest](docs/assets/lab-guide/live/cp2-v4-qa-entry-point-route.jpg). No completed interaction appeared in its Debug list during that test window. The entry point was then saved back to the original `ServiceDesk` **Latest** route. The route screenshot records configuration, not a completed phone test.
