...
| 9 | Replace the starter IVR in `ServiceDesk` with **Virtual Agent V2** and connect handled, escalated, errored, and queue-failure outcomes. | `ServiceDesk` version 5 is published as **Latest**, with zero validation errors. Active `Entry Point-1` routes to `ServiceDesk` **Latest**. A real version 5 order call at 20:48 tenant time followed `NewContact → AIAgent → DisconnectContact → ContactEnded`; a same-time AI Agent Studio Voice session ran `lookup_order` successfully. The session was matched by time, not by a shared interaction ID. The caller still needs to confirm the spoken order details. A separate general-support call at 21:46 tenant time reached `AIAgent → EscalationMessage → HumanAgentQueue → PlayMusic → PleaseWait → PlayMusic → ContactEnded`, all successful in Debug. The caller heard queue music; Analyze showed one execution and zero node errors in that call window. A human agent answer was not verified. |
...
1. Confirm with the caller that the version 5 order call spoke the expected status and delivery information. Its Debug path, same-time Voice-session `lookup_order` success, and Analyze result are already captured. Do not infer spoken wording from tool success alone.
2. Route the repaired practice version 3 in a controlled phone-test window, then call-test both its wait and callback branches. Restore `ServiceDesk` **Latest** to the entry point afterward. Keep the version 3 publication screenshot labeled as configuration evidence until those calls and Debug traces are captured.
3. If the direct REST exercise will be presented as a verified phone result, route its historical published version in a controlled test and capture its own Debug trace. The guide currently distinguishes this from the proven refactored lookup call.
4. Route historical `ServiceDesk` version 4 in a controlled window and call digit `2` to confirm the corrected direct-to-queue branch in Debug. Restore version 5 **Latest** afterward.
...
The repository was cloned and rendered locally before guide edits. Colleague PR [#1](https://github.com/WebexCC-SA/LAB-21170/pull/1) was analyzed and merged additively. Guide and media changes landed through PR [#2](https://github.com/WebexCC-SA/LAB-21170/pull/2), QA revision through PR [#3](https://github.com/WebexCC-SA/LAB-21170/pull/3), and the attendee-writing/media polish through PR [#4](https://github.com/WebexCC-SA/LAB-21170/pull/4). After PR #4, `main` and `arubhatt-flow-guide` both pointed to `74e0a93`. The strict MkDocs build, local preview, media/link checks, source and generated-site credential scan, and public Pages deployment passed for that revision.

Keep the local preview and annotation review available for feedback. The final voice order and human-handoff calls have been checked in Debug and Analyze; the spoken order details still need caller confirmation. The guide is complete when every asserted caller outcome has matching versioned evidence and each admin action has a safe screenshot. Publish later corrections through the feature branch and a separate PR.
